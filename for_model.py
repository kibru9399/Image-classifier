...
class classifier:
    ...
    def create_my_model(self, arch, learning_rate, hidden_state):
        ...
        classifier_part = nn.Sequential(nn.Linear(classifier_input, self.hidden_state),
                                nn.ReLU(),
                                nn.Dropout(0.5),
                                nn.Linear(self.hidden_state, 150),
                                nn.ReLU(),
                                nn.Linear(150, 120),
                                nn.ReLU(),
                                nn.Linear(120, 102),
    ...
    def predict(self, image_path, model, topk, predict_device):
        ...
        image = image.view(1, *image.shape).to(self.predict_device)
        model = model.to(self.predict_device)
        with torch.no_grad():
